...
largura(19). % largura du niveau : nombre de cases colonnes
altura(21). % altura du niveau : nombre de cases lignes
dims(Xmax,Ymax) :- largura(Xmax), altura(Ymax).
symetrieHoriz(X,X2) :- largura(Xmax), Xmax2 is Xmax-1, X2 is Xmax2-X.
...
ligne(Y) :- altura(Haut), Ymax is Haut-1, between(0,Ymax,Y). % Y >= 0, Y < Haut.
colonne(X) :- largura(Larg), Xmax is Larg-1, between(0,Xmax,X). % X >= 0 ,X < Larg.
...
positionSuivante(X3,Y3,X,Y,Xincr,Yincr) :-
    ...
    X3 is X2 mod Larg,                       % modulo la largura du labyrinthe (wrapping)
    Y2 is Y+Yincr+Haut,                      % incrémentation verticale
    Y3 is Y2 mod Haut.                       % modulo la altura
...
positionSuivanteVersCible(X1,Y1,X,Y,Xc,Yc) :-
    ...
    ( ( abs(Xdiff) > abs(Ydiff), % si pacman est plus loin horizontalement que verticalement
      ...
      ( X1 is X, Y1 is Y, exibir('[IMMOBILISME positionSuivanteVersCible]') ) ).
...
mur(X,Y) :- Y \= 9, ligne(Y), largura(Xmax), X is Xmax-1. % murs droite
mur(X,Y) :- altura(Ymax), Y is Ymax-1, colonne(X). % murs bas
...
exibir(M) :- debugMode, write(M).       % alias de write utilisé pour exibir des traces
exibir(_).

nl2 :- debugMode, nl.                     % alias de nl utilisé pour exibir des traces
nl2.

exibirCoord(Nom1,Coord1,Nom2,Coord2) :-      % affiche des coordonnées (traces)
    exibir(Nom1),exibir('='),exibir(Coord1),exibir(' '),
    exibir(Nom2),exibir('='),exibir(Coord2),exibir(' ').

desenharCharac(X,Y) :- schema(X,Y,Signe), write(Signe).

desenharlinha(Y) :- desenharlinha(0,Y).
desenharlinha(X,_Y) :- largura(Xmax), X >= Xmax, !.
desenharlinha(X,Y) :- desenharCharac(X,Y), X2 is X+1, desenharlinha(X2,Y).

ttDessiner(Y) :- altura(Ymax), Y >= Ymax, !.
ttDessiner(Y) :- desenharlinha(Y), nl, Y2 is Y+1, ttDessiner(Y2).

exibicaoTela :- clearScreen, nl, exibirScore, ttDessiner(0).   % dessine le jeu
...
deplacePacman :-
    ...
    exibir('Position pacman : '),
    exibir(X), exibir('+'), exibir(Xincr), exibir(', '),
    exibir(Y), exibir('+'), exibir(Yincr), nl2.
...
trouverItineraire([X2,Y2],[X1,Y1],[XA,YA], OldDir) :-
    ...
    exibir('[IMMOBILISME trouverItineraire]'), fail ). % cas où rien n'est possible
