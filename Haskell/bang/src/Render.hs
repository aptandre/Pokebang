...
render image_bulbasaur image_charmander image_pokeball foreground image_vileplum image_slowpoke image_stone game@Game { gameState = Playing }
 ...
 where
  frame = pictures
    ...
    ++ [makeVileplumLeftBall image_pokeball game]
    ++ [makeVilePlumRightBall image_pokeball game]
    ++ [(makeObstacleVilePlum image_vileplum) (vileplume game)]
...
makeVileplumLeftBall picture game = makePokeBall (vilePlumShootLeft $ vileplume game) picture

makeVilePlumRightBall :: Picture -> BANG -> Picture
makeVilePlumRightBall picture game = makePokeBall (vilePlumShootRight $ vileplume game) picture